run this for launching gazebo with husky: 

roslaunch /opt/ros/noetic/share/husky_gazebo/launch/empty_world.launch

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
code without service:
#!/usr/bin/env python3

import rospy  # Import the rospy library for ROS in Python
...
def main():
    ...
    try:
        while not rospy.is_shutdown():
            # Publish the Twist message on the '/husky_velocity_controller/cmd_vel' topic for 3 seconds
            for i in range(30):
                vel_pub.publish(vel_cmd)
                rate.sleep()

            # Stop the robot by publishing a Twist message with a linear x velocity of 0 for 2 seconds
            vel_cmd.linear.x = 0
            vel_pub.publish(vel_cmd)
            for i in range(20):
                rate.sleep()

            # Reset the Twist message linear x velocity to 0.5 to start moving the robot again
            vel_cmd.linear.x = 0.5

    except rospy.ROSInterruptException:
        pass

    # After the try-except block, set the linear x velocity to 0 to stop the robot
    vel_cmd.linear.x = 0
    vel_pub.publish(vel_cmd)
